...
class Parser(object):
    ...
    def _py_expr_(self):
        ...
        def choice_0():
            ...
            if self.err:
                self._pop('py_expr_0')
                return
            self.val = ['py_plus', self._get('e1'), self._get('e2')]
            self.err = None
            self._pop('py_expr_0')
        choice_0()
        if not self.err:
    ...
    def _py_qual_(self):
        ...
        def choice_0():
            ...
            if self.err:
                self._pop('py_qual_0')
                return
            self.val = ['py_qual', self._get('e'), self._get('ps')]
            self.err = None
            self._pop('py_qual_0')
        choice_0()
        if not self.err:
    ...
    def _py_post_op_(self):
        p = self.pos
        def choice_0():
            self._push('py_post_op_0')
            self._expect('[')
            if self.err:
                self._pop('py_post_op_0')
                return
            self._sp_()
            if self.err:
                self._pop('py_post_op_0')
            ...
            if self.err:
                self._pop('py_post_op_0')
                return
            self.val = ['py_getitem', self._get('e')]
            self.err = None
            self._pop('py_post_op_0')
        choice_0()
        if not self.err:
        ...
        self.pos = p
        def choice_1():
            self._push('py_post_op_1')
            self._expect('(')
            if self.err:
                self._pop('py_post_op_1')
                return
            self._sp_()
            if self.err:
                self._pop('py_post_op_1')
        ...
        self.pos = p
        def choice_2():
            self._push('py_post_op_2')
            self._expect('.')
            if self.err:
                self._pop('py_post_op_2')
                return
            self._ident_()
            if not self.err:
                self._set('i', self.val)
            if self.err:
                self._pop('py_post_op_2')
                return
            self.val = ['py_getattr', self._get('i')]
            self.err = None
            self._pop('py_post_op_2')
        choice_2()
    ...
    def _py_prim_(self):
        ...
        def choice_2():
            ...
            if self.err:
                self._pop('py_prim_2')
                return
            self.val = ['py_num', '0x' + self._get('hs')]
            self.err = None
            self._pop('py_prim_2')
        choice_2()
        if not self.err:
    ...
    def _py_exprs_(self):
        p = self.pos
        def choice_0():
            self._push('py_exprs_0')
            self._py_expr_()
            if not self.err:
                self._set('e', self.val)
            if self.err:
                self._pop('py_exprs_0')
                return
            vs = []
            while not self.err:
                p = self.pos
